...
import { FiPlus, FiCreditCard, FiTrash2, FiEdit2, FiCheckCircle } from 'react-icons/fi';
import CardForm from './CardForm';

const PaymentStep = ({ axiosWithAuth, onSelectCard }) => {
  ...
  const fetchCards = async () => {
    try {
      const res = await axiosWithAuth.get('/user/card');
      setCards(res.data);
    } catch (err) {
  ...
  const deleteCard = async (id) => {
    try {
      await axiosWithAuth.delete(`/user/card/${id}`);
      fetchCards();
    } catch (err) {
